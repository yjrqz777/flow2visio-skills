...
当前技能版本：0.5.5。首次进度消息必须报告“已加载 flow2visio v0.5.5”。
...
无 RTOS 项目以 `main` 或等价入口作为模块识别的第一依据。先按源码顺序完整枚举 `main` 初始化阶段和长期 `while`、`for` 或 `do` 主循环中的直接调用、宏调用、周期判断、状态机分发、任务表、函数参数及回调注册，再补充由这些入口关联的中断、定时器和异步回调；不得先凭目录或函数大小筛选模块。

凡在 `main` 中直接调用、作为参数传入、注册到调度器/事件/定时器或能被后续触发的项目自有处理入口，默认都视为正式模块，必须进入模块清单并追踪实际实现。`RunTaskA(callback, slot)`、类似时间片封装以及函数指针表的每个注册项均为独立执行入口；不得因调用形式重复、没有注释、函数较短或多个入口使用同一封装而省略。注释与符号或实现冲突时，以实际代码为准并记录冲突。第三方库、芯片厂商生成代码和语言运行库入口可以排除，但必须在忽略项中说明，不能把项目自有入口当作普通辅助函数排除。

项目主流程或主循环/调度总览必须为每个入口显示独立节点；每个不同的项目自有实际处理入口还必须生成一张独立模块流程页，页内说明该入口被触发后自身执行的判断、状态、时序、输入、输出和结束方式。模块页数量应与去重后的项目自有实际入口数量一致。多个调用位置只有指向同一个实际入口时才能共用该入口的模块页，但总览和覆盖表仍分别保留各触发位置；不得把不同入口合并到同一模块页。
...
在 `project-overview.md` 建立“main入口覆盖表”，至少记录调用或注册位置、时间片或触发条件、实际入口、模块职责、所在页面和源码依据。完成前将覆盖表逐项反查 `main` 的初始化和主循环，确认每个项目自有入口都对应独立页面；间接目标无法确定时标为不确定并保留页面，不得删除。
...
主要执行上下文是：无 RTOS 的 `main`、主循环和每个可触发项目入口；RTOS 的启动过程和每个已创建或注册任务。只在模块内部调用的普通辅助函数不增加页面，但出现在 `main` 调用、参数或注册关系中的项目自有入口绝不能归类为普通辅助函数。无法判断是否使用 RTOS 时，在文档中说明依据和不确定点，不得猜测任务关系。
...
页面默认依次为：软件架构总览、项目主流程、主循环或任务关系、每个 main 项目入口或 RTOS 任务的独立流程、必要子流程、可选存储器布局。项目页只展示入口、主要执行上下文和阶段切换；任务或模块页必须依据实际实现说明被触发后的内部工作方式。
