---
name: flow2visio
description: Generate editable Microsoft Visio flowcharts from user requirements by writing Graphviz DOT, converting DOT to Graphviz plain output, and converting plain output to .vsdx with the bundled Graphviz2Visio-win-x64-standalone tool. Use when Codex needs to create, modify, or export flow diagrams, process maps, decision trees, workflow charts, or Visio files.
---
...
2. Convert DOT to Graphviz plain format with `Graphviz2Visio-win-x64-standalone dot2plain`.
3. Convert plain format to `.vsdx` with `Graphviz2Visio-win-x64-standalone plain2visio`.
...
```text
scripts/Graphviz2Visio-win-x64-standalone.exe
```

This direct `scripts/Graphviz2Visio-win-x64-standalone.exe` layout is appropriate when the C# CLI was published as a single-file executable. If the CLI is published as a framework-dependent or multi-file build, place the whole published output folder under `scripts/Graphviz2Visio-win-x64-standalone/` instead and keep sibling files such as `.dll`, `.deps.json`, `.runtimeconfig.json`, and native dependencies next to the `.exe`.

If the bundled CLI is absent, check whether `Graphviz2Visio-win-x64-standalone` is available on `PATH`. If neither is available, ask the user to place the published CLI executable under `scripts/` or the published CLI folder under `scripts/Graphviz2Visio-win-x64-standalone/`.
...
scripts\Graphviz2Visio-win-x64-standalone.exe dot2plain flow\diagram.dot flow\diagram.plain
scripts\Graphviz2Visio-win-x64-standalone.exe plain2visio flow\diagram.plain flow\diagram.vsdx
```
